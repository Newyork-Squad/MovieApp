    ...
    override fun getData() {
        viewModelScope.launch {
            try {
                ...
                val genreListId = if (mediaTypeStr == "movie") Constants.MOVIE_CATEGORIES_ID else Constants.TV_CATEGORIES_ID
                val allGenres = getGenreListUseCase(genreListId)

                val allMediaItems = getMediaByType(args.type, args.id)
                    .map { pager ->
                        pager.map { media ->
                ...
                _uiState.update { it.copy(error = listOf(Error(404, t.message ?: "Unknown error"))) }
            }
        }
    }
