...
class MatchQuestionsViewModel @Inject constructor(
    ...
    MatchQuestionInteractionListener, MatchResultInteractionListener  {
    ...
    fun onChoiceSelected(type: MatchQuestionType, choices: List<Choice>) {
        _uiState.update { state ->
            when (type) {
                ...
                MatchQuestionType.MEDIA_RUNTIME -> {
                    state.copy(
                        mediaRuntimeSelected = choices.mapIndexed { index, choice ->
                ...
                MatchQuestionType.TIME_PERIOD -> {
                    state.copy(
                        timePeriodSelected = choices.mapIndexed { index, choice ->
                            choice.copy(isSelected = index == 0)
                        }
                    )
                }
        ...
        viewModelScope.launch {
            try {
                ...
                _uiState.update {
                    it.copy(
                        error = listOf(
                            MatchQuestionUiState.ErrorUIState(
    ...
    private suspend fun getMovieDetails(movieId: Int): MatchQuestionUiState.MovieUIState {
        ...
        return MatchQuestionUiState.MovieUIState(
    ...
    override fun onClickViewDetails() {
        _matchResultUiEvent.update { Event(MatchUiEvent.ViewMovieDetails) }
    }
    ...
    override fun onClickYoutubeTrailer(movieId: Int) {
        _matchResultUiEvent.update { Event(MatchUiEvent.PlayYoutubeTrailer(movieId)) }
    }

    override fun onClickSaveMovie(movieId: Int) {
        _matchResultUiEvent.update { Event(MatchUiEvent.SaveMovie(movieId)) }
    }
